...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
        ...
        self.fc1 = QLinearLayer(in_features, hidden_features, args=args, layer_type="mlp_fc1")   # self.fc1 = nn.Linear(in_features, hidden_features)
        self.act = act_layer()
        self.fc2 = QLinearLayer(hidden_features, out_features, args=args, layer_type="mlp_fc2")  # self.fc2 = nn.Linear(hidden_features, out_features)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        self.qkv = QLinearLayer(dim, dim * 3, bias=qkv_bias, args=args, layer_type="attn_qkv")   # self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = QLinearLayer(dim, dim, args=args, layer_type="attn_proj")                    # self.proj = nn.Linear(dim, dim)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, args=None):
        super().__init__()
