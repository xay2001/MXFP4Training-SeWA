...
MODEL_NAME="deit_small"         # Model name in ["deit_tiny", "deit_small", "deit_base"]
                                # NOTE: "deit_base" needs a different Learning-Rate & Batch-Size setting
OMP_NUM_THREADS=8

alpha=16
beta=5
Experiment_NAME="TetraJet-MXFP4-Ramping${alpha}-${beta}"
...
python -m torch.distributed.run --nproc_per_node=4 --master_port=29501 main.py \
    ...
    --opt adamwBSRamping \
    --opt-ramping-alpha $alpha \
    --opt-ramping-beta $beta \
    --opt-savepath ${SAVE_PATH}/tmp_model.pth \
