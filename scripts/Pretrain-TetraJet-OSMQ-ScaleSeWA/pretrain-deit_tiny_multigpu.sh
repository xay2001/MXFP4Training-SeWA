...
EMA_DECAY=0.9983
# 4 GPUs: 50000 starts near epoch 40. For 2 GPUs, use about 100000.
OSMQ_START_STEP=${OSMQ_START_STEP:-100000}
OSMQ_TAU=1.0
SCALE_SEWA_TAU=1.0
...
Experiment_NAME="TetraJet-MXFP4-OSMQ-ScaleHistory-EMA${EMA_DECAY}-S${OSMQ_START_STEP}-ST${SCALE_SEWA_TAU}-SI${SCALE_UPDATE_INTERVAL}-Multi"
...
CUDA_VISIBLE_DEVICES=${GPUS} python -m torch.distributed.run --nproc_per_node=${NPROC_PER_NODE} --master_port=${MASTER_PORT} main.py \
