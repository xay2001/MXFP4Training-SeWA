...
OSMQ_TAU=0.5
OSMQ_CURRENT_BIAS=-2.0
Experiment_NAME="TetraJet-MXFP4-OSMQ-SmoothStart-EMA${EMA_DECAY}-S${OSMQ_START_STEP}-CB${OSMQ_CURRENT_BIAS}-T${OSMQ_TAU}"
...
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.run --nproc_per_node=4 --master_port=29503 main.py \
    ...
    --qlinear-osmq-init-current-bias ${OSMQ_CURRENT_BIAS} \
